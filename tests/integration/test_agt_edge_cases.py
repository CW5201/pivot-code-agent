...
from pivotcode.git_tree.model import CURRENT_NODE_SHA
from pivotcode.git_tree.operations import agt_move, agt_revert
from pivotcode.git_tree.memory_snapshots import take_memory_snapshot
from pivotcode.git_tree.parser import parse_git_tree
from pivotcode.gui.scripted_ui import ScriptedUI
from pivotcode.messages.types import UserMessage
...
class TestEC01RevertWithSHA:
    ...
    async def test_revert_sha_destroys_commits(self, git_repo: GitTestRepo):
        ...
        assert len(agent._session.pivot_commits) == 0  # Both cleaned
...
class TestEC03CommitMessageIndices:
    ...
    async def test_index_recorded_on_commit(self, git_repo: GitTestRepo):
        ...
        for sha in agent._session.pivot_commits:
...
class TestEC05ResumeSession:
    ...
    async def test_agt_state_persists_across_sessions(self, git_repo: GitTestRepo):
        """AGT state (pivot_commits, conv_path) survives session restart."""
        git_repo.write_file("f.py", "x")
        ...
        assert agent2._session.pivot_commits == saved_commits
    ...
    async def test_resumed_session_tree_update(self, git_repo: GitTestRepo):
        ...
        # Tree update should include the pivot commit from session 1
...
class TestEC06ExternalCommitBetweenTurns:
    ...
    async def test_external_commit_detected(self, git_repo: GitTestRepo):
        ...
        agent = PivotCodeAgent(backend=provider, cwd=str(git_repo.path))
...
class TestEC07RevertPastMerge:
    ...
    def test_revert_past_merge(self, git_repo: GitTestRepo):
        ...
        from pivotcode.git_tree.operations import agt_revert
...
class TestEC08DuplicateSHAsInConvPath:
    ...
    def test_convrevert_with_duplicates(self, git_repo: GitTestRepo):
        ...
        from pivotcode.git_tree.operations import agt_conv_revert
...
class TestEC10MemodiffContent:
    ...
    async def test_memodiff_shows_changes(self, git_repo: GitTestRepo):
        ...
        # Set up agent with these as pivot_commits
        _, ui, agent = _make_session(git_repo, [], ["/memodiff", EOFError])
        agent._session.pivot_commits = [sha1, sha2]
