...
class TestSessionStateAGT:
    ...
    def test_pivot_commits_default(self, tmp_path):
        state = SessionState(session_id="test", cwd=str(tmp_path))
        assert state.pivot_commits == []

    def test_pivot_commits_set_get(self, tmp_path):
        state = SessionState(session_id="test", cwd=str(tmp_path))
        state.pivot_commits = ["abc123", "def456"]
        assert state.pivot_commits == ["abc123", "def456"]

    def test_add_pivot_commit(self, tmp_path):
        state = SessionState(session_id="test", cwd=str(tmp_path))
        state.add_pivot_commit("abc123")
        state.add_pivot_commit("def456")
        assert state.pivot_commits == ["abc123", "def456"]
    ...
    def test_backward_compat(self, tmp_path):
        ...
        import json
        state_dir = tmp_path / ".pivot" / "sessions" / "old"
...
class TestGitCommitTool:
    ...
    async def test_updates_session_state(self, git_repo: GitTestRepo):
        ...
        assert sha in state.pivot_commits
