...
from pivotcode.providers.scripted_provider import ScriptedProvider, ScriptedResponse, text, tool_call
...
class TestTextOnlyResponse:
    ...
    async def test_simple_text_response(self):
        ...
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test")
...
class TestMultiTurn:
    ...
    async def test_agent_state_returns_to_waiting(self):
        from pivotcode.agent import AgentState
...
class TestMaxTurns:
    ...
    async def test_stops_at_max_iterations_per_turn(self):
        ...
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test", max_iterations_per_turn=3)
