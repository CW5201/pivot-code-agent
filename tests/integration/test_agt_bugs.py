...
Bug catalog:
  BUG-01: git clean -fd deleted .pivot/ directory
  ...
  BUG-08: State write failures when .pivot not gitignored
...
from pivotcode.git_tree.model import CURRENT_NODE_SHA, NodeType
...
from pivotcode.gui.scripted_ui import ScriptedUI, ui_rule
from pivotcode.messages.types import AssistantMessage, UserMessage
...
# ═══════════════════════════════════════════════════════════════════════════════
# BUG-01: git clean -fd must not delete .pivot/
# ═══════════════════════════════════════════════════════════════════════════════


class TestBug01AlanDirSurvivesGitClean:
    """git operations in agt_move/agt_revert must preserve .pivot/."""
    ...
    def test_move_preserves_alan_dir(self, git_repo: GitTestRepo):
        ...
        # Write something to .pivot that must survive
        alan_file = git_repo.pivot_dir() / "important.txt"
        alan_file.write_text("must survive")

        agt_move(str(git_repo.path), state, shas[0])

        assert alan_file.exists(), ".pivot/important.txt was deleted by agt_move!"
    ...
    def test_revert_preserves_alan_dir(self, git_repo: GitTestRepo):
        ...
        alan_file = git_repo.pivot_dir() / "sessions" / "b01r" / "state.json"
...
class TestBug04RevertDestroysCommits:
    ...
    def test_revert_cleans_pivot_commits(self, git_repo: GitTestRepo):
        shas = git_repo.build_linear_history(3)
        state = SessionState(session_id="b04c", cwd=str(git_repo.path))
        state.agent_position_sha = shas[2]
        state.pivot_commits = [shas[1], shas[2]]

        agt_revert(str(git_repo.path), state, 1)

        assert shas[2] not in state.pivot_commits
        assert shas[1] in state.pivot_commits
...
# ═══════════════════════════════════════════════════════════════════════════════
# BUG-08: .pivot must be gitignored for safety
# ═══════════════════════════════════════════════════════════════════════════════


class TestBug08AlanGitignored:
    """Agent must ensure .pivot/ is gitignored on session start."""

    @pytest.mark.asyncio
    async def test_gitignore_created_if_missing(self, tmp_path):
        """If no .gitignore exists, one is created with .pivot/."""
        ...
        agent = PivotCodeAgent(backend=provider, cwd=str(repo.path))
        await run_session(agent, ui)

        gitignore = repo.path / ".gitignore"
        assert gitignore.exists()
        assert ".pivot" in gitignore.read_text()

    @pytest.mark.asyncio
    async def test_gitignore_appended_if_missing_entry(self, tmp_path):
        """If .gitignore exists but doesn't have .pivot, it's appended."""
...
class TestBug10OrphanedSHAs:
    ...
    def test_no_false_positives(self, git_repo: GitTestRepo):
        ...
        state.pivot_commits = [shas[0], shas[1]]
...
class TestTreeUpdates:
    ...
    @pytest.mark.asyncio
    async def test_tree_pivot_commits_blue(self, git_repo: GitTestRepo):
        git_repo.write_file("f.py", "x")
        _, ui, agent = _make_session(git_repo, [
            tool_call("GitCommit", {"message": "C1"}), text("OK"),
        ], ["Commit", EOFError])
        ...
        tree = ui.tree_update_log[-1]
        alan_nodes = [n for n in tree["nodes"] if n["node_type"] == "pivot_commit"]
    ...
    async def test_no_tree_for_non_git(self, tmp_path):
        ...
        agent = PivotCodeAgent(backend=provider, cwd=str(tmp_path))
...
class TestFullScenario:
    ...
    async def test_commit_revert_commit_move(self, git_repo: GitTestRepo):
        ...
        assert len(agent._session.pivot_commits) >= 2
