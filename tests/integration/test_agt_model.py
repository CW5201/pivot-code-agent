...
class TestAGTNodeBasics:
    ...
    def test_node_to_dict(self):
        node = AGTNode(
            ...
            parents=["parent1"],
            node_type=NodeType.PIVOT_COMMIT,
            branches=["main"],
            is_head=True,
        )
        d = node.to_dict()
        assert d["sha"] == "abc123def456"
        assert d["node_type"] == "pivot_commit"
...
class TestAlanVsExternalClassification:
    """Test that pivot_commits set correctly classifies nodes."""

    def test_classification(self, git_repo: GitTestRepo):
        shas = git_repo.build_linear_history(3)

        # Mark shas[1] as pivot commit
        tree = parse_git_tree(str(git_repo.path), pivot_commits={shas[1]})

        alan_node = tree.get_node(shas[1])
        assert alan_node is not None
        assert alan_node.node_type == NodeType.PIVOT_COMMIT
    ...
    def test_all_alan(self, git_repo: GitTestRepo):
        shas = git_repo.build_linear_history(3)
        tree = parse_git_tree(str(git_repo.path), pivot_commits=set(shas))
    ...
    def test_no_alan(self, git_repo: GitTestRepo):
        shas = git_repo.build_linear_history(3)
        tree = parse_git_tree(str(git_repo.path))  # No pivot_commits
...
class TestToJson:
    ...
    def test_serializable(self, git_repo: GitTestRepo):
        ...
        # Find the pivot commit in nodes
        alan_nodes = [n for n in j["nodes"] if n["node_type"] == "pivot_commit"]
