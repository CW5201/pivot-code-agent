...
class TestAgentCommitInTree:
    ...
    async def test_commit_creates_alan_node(self, git_repo: GitTestRepo):
        ...
        assert sha in agent._session.pivot_commits
    ...
    async def test_tree_update_sent_after_commit(self, git_repo: GitTestRepo):
        ...
        alan_nodes = [n for n in nodes if n["node_type"] == "pivot_commit"]
...
class TestMoveCommand:
    ...
    async def test_move_creates_branch_on_non_tip(self, git_repo: GitTestRepo):
        ...
        assert any("pivot/move" in b for b in branches)
...
class TestTreeUpdateContent:
    ...
    async def test_tree_has_correct_structure(self, git_repo: GitTestRepo):
        ...
        for node in tree["nodes"]:
            ...
            assert node["node_type"] in ("pivot_commit", "external", "current")
...
class TestEdgeCases:
    ...
    async def test_revert_then_commit_on_new_branch(self, git_repo: GitTestRepo):
        ...
        assert "pivot" in branch or branch == "main"

        # New commit should be an pivot commit
        if agent._session.pivot_commits:
            new_sha = agent._session.pivot_commits[-1]
            tree = parse_git_tree(str(git_repo.path), set(agent._session.pivot_commits))
            node = tree.get_node(new_sha)
            assert node is not None
            assert node.node_type == NodeType.PIVOT_COMMIT
    ...
    async def test_commit_and_revert_roundtrip(self, git_repo: GitTestRepo):
        ...
        # Should have 2 pivot commits
        assert len(agent._session.pivot_commits) >= 1
