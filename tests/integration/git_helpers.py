...
class GitTestRepo:
    ...
    def init(self, initial_branch: str = "main") -> None:
        ...
        self._run("git", "config", "user.email", "test@pivotcode.dev")
        self._run("git", "config", "user.name", "Test User")
        # Create .pivot/ directory (gitignored)
        alan_dir = self.path / ".pivot"
        alan_dir.mkdir(exist_ok=True)
        (self.path / ".gitignore").write_text(".pivot/\n")
    ...
    # ── Pivot-specific helpers ────────────────────────────────────────

    def pivot_dir(self) -> Path:
        """Return the .pivot/ directory path."""
        d = self.path / ".pivot"
        d.mkdir(exist_ok=True)
        return d

    def memory_dir(self) -> Path:
        """Return the .pivot/memory/ directory path."""
        d = self.pivot_dir() / "memory"
    ...
    def memory_snapshot_dir(self, commit_sha: str) -> Path:
        ...
        d = self.pivot_dir() / "memory_snapshots" / commit_sha
        d.mkdir(parents=True, exist_ok=True)
        return d
