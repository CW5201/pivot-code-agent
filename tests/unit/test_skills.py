...
from pivotcode.skills.parser import SkillDefinition, parse_skill_content, parse_skill_file
from pivotcode.skills.discovery import discover_skills, _scan_skills_dir
...
def test_discover_skills_project_overrides_builtin(tmp_path):
    ...
    skills_dir = tmp_path / ".pivot" / "skills" / "create"
...
def test_discover_skills_project_and_user(tmp_path, monkeypatch):
    ...
    user_skills = user_home / ".pivot" / "skills" / "deploy"
    ...
    proj_skills = project / ".pivot" / "skills" / "deploy"
...
def test_registry_expand(tmp_path):
    skills_dir = tmp_path / ".pivot" / "skills" / "greet"
    skills_dir.mkdir(parents=True)
    (skills_dir / "SKILL.md").write_text(textwrap.dedent("""\
        ---
        name: greet
...
def test_registry_reload(tmp_path):
    ...
    # Add a new skill
    skills_dir = tmp_path / ".pivot" / "skills" / "new-skill"
    skills_dir.mkdir(parents=True)
    (skills_dir / "SKILL.md").write_text(textwrap.dedent("""\
        ---
        name: new-skill
