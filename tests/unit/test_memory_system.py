...
from pivotcode.memory.prompt import (
...
from pivotcode.settings import SETTINGS_DEFAULTS, load_settings, save_settings
...
class TestMemoryDirectoryStructure:
    ...
    def test_get_memory_dir_returns_alan_memory(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result.parts[-2:] == (".pivot", "memory")
            assert str(result).startswith(tmpdir)

    def test_ensure_memory_structure_creates_all_subdirs(self):
        from pivotcode.memory.memdir import PROJECT_MEMORY_SUBDIRS, GLOBAL_MEMORY_SUBDIRS, get_global_memory_dir
        with tempfile.TemporaryDirectory() as tmpdir:
...
class TestScratchpad:
    ...
    def test_get_scratchpad_dir_path(self):
        ...
        assert str(result).endswith(os.path.join(".pivot", "sessions", "abc123", "scratchpad"))

    def test_cleanup_old_scratchpads_removes_oldest(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            sessions_root = os.path.join(tmpdir, ".pivot", "sessions")
...
class TestPromptBuilding:
    ...
    def test_on_mode_contains_key_sections(self):
        result = get_memory_instructions_on("/tmp/.pivot/memory")
        ...
        assert "/tmp/.pivot/memory" in result
    ...
    def test_intensive_mode_proactive(self):
        result = get_memory_instructions_intensive("/tmp/.pivot/memory")
