"""Test the cost tracker (pivotcode/api/cost_tracker.py)."""
...
from pivotcode.api.cost_tracker import (
    ANTHROPIC_PRICING,
    DASHSCOPE_PRICING,
    CostTracker,
    _anthropic_cost,
...
# ---------------------------------------------------------------------------
# DashScope cost calculation
# ---------------------------------------------------------------------------


class TestDashScopeCost:

    def test_known_model_exact_match(self):
        """Known model name returns a non-None cost."""
        usage = Usage(input_tokens=1000, output_tokens=500)
        cost = _dashscope_cost(usage, "qwen-plus")
        assert cost is not None
        prices = DASHSCOPE_PRICING["qwen-plus"]
        expected = (1000 * prices["input"] + 500 * prices["output"]) / 1_000_000
        assert cost == pytest.approx(expected)

    def test_prefix_match(self):
        """Model name with suffix matches via prefix."""
        usage = Usage(input_tokens=1000, output_tokens=500)
        cost = _dashscope_cost(usage, "qwen-plus-latest")
        assert cost is not None
        cost_base = _dashscope_cost(usage, "qwen-plus")
        assert cost == pytest.approx(cost_base)

    def test_unknown_model_returns_none(self):
        """Non-DashScope model returns None."""
        usage = Usage(input_tokens=100, output_tokens=50)
        cost = _dashscope_cost(usage, "claude-sonnet-4-6")
        assert cost is None

    def test_zero_usage_returns_zero(self):
        """Zero tokens yields zero cost."""
        usage = Usage()
        cost = _dashscope_cost(usage, "qwen-max")
        assert cost == 0.0

    def test_qwen_turbo_cheaper(self):
        """qwen-turbo should be cheaper than qwen-plus."""
        usage = Usage(input_tokens=1000, output_tokens=500)
        cost_turbo = _dashscope_cost(usage, "qwen-turbo")
        cost_plus = _dashscope_cost(usage, "qwen-plus")
        assert cost_turbo is not None and cost_plus is not None
        assert cost_turbo < cost_plus
