...
from pivotcode.providers.remote_scripted_provider import RemoteScriptedProvider
...
def test_disk_mirror_when_bound(provider, tmp_path):
    ...
    ``<cwd>/.pivot/sessions/<sid>/remote_inbox.json`` once bound."""
    provider.set_session_context(session_id="deadbeef" + "0" * 24, cwd=str(tmp_path))
    expected = (
        tmp_path / ".pivot" / "sessions" / ("deadbeef" + "0" * 24) / "remote_inbox.json"
    )
...
def test_no_disk_mirror_when_unbound(provider, tmp_path):
    ...
    asyncio.run(run())
    # No .pivot/ created since we never bound a session.
    assert not (tmp_path / ".pivot").exists()


def test_response_error_yields_stream_error(provider):
    from pivotcode.providers.base import StreamError
...
def test_resolve_backend_picks_remote_provider():
    from pivotcode.agent import _resolve_backend
