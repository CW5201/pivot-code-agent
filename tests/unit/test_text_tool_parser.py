...
from pivotcode.tools.text_tool_parser import (
...
class TestPivotFormat:
    """Pivot's custom text tool call format."""
    ...
    def test_single_tool_call(self):
        ...
        result = extract_tool_calls_from_text(text, format="pivot")
        assert len(result.tool_calls) == 1
        assert result.tool_calls[0].name == "Bash"
        assert result.error is None
...
class TestMalformedDetection:
    ...
    def test_bare_tool_call_tag_in_prose_not_flagged(self):
        ...
        for fmt in ("hermes", "glm", "pivot"):
            tag = "<tool_use>" if fmt == "pivot" else "<tool_call>"
...
class TestSystemPrompt:
    ...
    def test_alan_prompt(self):
        schemas = [{"type": "function", "function": {"name": "Bash", "description": "Run command", "parameters": {}}}]
        prompt = get_tool_format_system_prompt("pivot", schemas)
