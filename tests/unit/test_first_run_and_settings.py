...
class TestLegacyProviderMigration:
    """The migrate_legacy_provider_key helper handles old .pivot/settings.json."""
...
class TestUpdateSessionSettingRejectsRestartRequired:
    ...
    def test_backend_change_recreates_provider(self):
        ...
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test_prov_change", permission_mode="yolo")
    ...
    def test_legacy_provider_key_still_accepted(self):
        ...
        provider = ScriptedProvider()
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test_legacy_set", permission_mode="yolo")
    ...
    def test_tool_call_format_change_succeeds(self):
        ...
        provider = ScriptedProvider()
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test_tcf", permission_mode="yolo")
    ...
    def test_allow_model_change(self):
        ...
        provider = ScriptedProvider()
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test_allow1", permission_mode="yolo")
    ...
    def test_allow_memory_change(self):
        ...
        provider = ScriptedProvider()
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test_allow2", permission_mode="yolo")
    ...
    def test_allow_permission_mode_change(self):
        ...
        provider = ScriptedProvider()
        agent = PivotCodeAgent(backend=provider, cwd="/tmp/test_allow3", permission_mode="yolo")
...
class TestUpdateProjectSetting:
    ...
    def test_allow_backend_change_in_project(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            provider = ScriptedProvider()
            agent = PivotCodeAgent(backend=provider, cwd=tmpdir, permission_mode="yolo")
...
class TestFirstRunDetection:
    ...
    def test_legacy_settings_file_is_migrated(self):
        """An old .pivot/settings.json with the 'provider' key loads cleanly."""
        with tempfile.TemporaryDirectory() as tmpdir:
            from pathlib import Path

            alan_dir = Path(tmpdir) / ".pivot"
            alan_dir.mkdir()
...
class TestAPIKeyDetection:
    ...
    def test_detect_anthropic_key(self):
        from pivotcode.cli.main import _detect_api_keys
    ...
    def test_detect_dashscope_key(self):
        from pivotcode.cli.main import _detect_api_keys
        ...
        try:
            ...
            detections = _detect_api_keys()
            dashscope_found = any(
                d.get("model", "").startswith("dashscope/") or d.get("model") == "qwen-plus"
                for d in detections
            )
            assert dashscope_found
        finally:
            if old:
...
class TestValidatorCoverage:
    ...
    def test_all_backend_settings_have_validators(self):
        from pivotcode.settings import SETTING_VALIDATORS
