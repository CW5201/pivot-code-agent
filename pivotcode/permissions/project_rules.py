...
per-session state file (``.pivot/sessions/<id>/state.json``), which meant
...
File: ``<project>/.pivot/allow_rules.json``
Format:
    [
...
from pivotcode.settings import get_pivot_dir
...
def _rules_path(cwd: str | None = None) -> Path:
    return get_pivot_dir(cwd) / _FILENAME


def load_project_allow_rules(cwd: str | None = None) -> list[dict[str, Any]]:
    """Read ``.pivot/allow_rules.json``. Returns ``[]`` if missing or invalid."""
...
def save_project_allow_rules(
    ...
    """Write the full list of allow rules to ``.pivot/allow_rules.json``."""
