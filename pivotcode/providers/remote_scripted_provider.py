...
一个测试/模拟后端，托管一个小型 HTTP 服务器并等待外部调用者
（人类或其他代理）提交助手响应。

通过以下方式选择::

    PivotCodeAgent(backend="scripted", model="remote")

或在命令行::

    pivotcode --backend scripted --model remote

端点（主机：``127.0.0.1``，端口在启动时选择并打印到标准输出）：

- ``GET  /api/health``      — 服务器启动后返回 ``{"ok": true}``。
- ``GET  /api/session``     — 会话元数据（id、cwd、model）。
- ``GET  /api/pending``     — 当前待处理的 LLM 调用负载，或 ``204``。
- ``POST /api/respond``     — 提交助手响应，解除 ``stream()`` 的阻塞。

相同的待处理负载也会镜像到磁盘
``<cwd>/.pivot/sessions/<session_id>/remote_inbox.json``（在代理绑定
其会话上下文后，参见 ``set_session_context``）。该镜像为只读 —
写入通过 ``POST /api/respond`` 进行。
"""
...
class RemoteScriptedProvider(LLMProvider):
    """将每次 LLM 调用委托给外部 HTTP 调用者的服务提供者。

    每次 ``stream()`` 时，调用负载通过 ``GET /api/pending`` 暴露，
    该方法阻塞直到收到 ``POST /api/respond``。响应 JSON 被转换为
    与常规 ``ScriptedProvider`` 发出的相同的服务提供者流事件序列。
    """
    ...
    def __init__(self, *, port: int | None = None) -> None:
        ...
        # 跨线程通信：HTTP 处理程序线程完成此 future
        # 以将响应传递回等待的 ``stream()`` 协程。
    ...
    def _mirror_to_disk(self, payload: dict[str, Any]) -> None:
        if not self._cwd or not self._session_id:
            return
        target = (
            Path(self._cwd) / ".pivot" / "sessions" / self._session_id
            / "remote_inbox.json"
        )
        try:
