"""随 Pivot Code 一同发布的内置技能。

这些是直接在 Python 中定义的 SkillDefinition 对象，而非来自磁盘文件。
它们作为默认值，可被用户或项目的技能覆盖。
"""

from pivotcode.skills.parser import SkillDefinition

# ── /skill create —— 交互式技能生成器 ─────────────────────────────
...
- **Existing skill**: Use the Read or Glob tool to check if
  `.pivot/skills/<name>/SKILL.md` already exists in the project directory. If it does,
...
Write it to `.pivot/skills/<name>/SKILL.md` in the project directory.

Note: If the user wants a global skill (available across all projects), they should
manually place the SKILL.md at `~/.pivot/skills/<name>/SKILL.md` instead. The /skill
