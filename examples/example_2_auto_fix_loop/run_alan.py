"""Auto-fix loop: Pivot fixes bugs iteratively until tests pass.

Demonstrates using PivotCodeAgent as a library — no async, no CLI, just a
simple Python script that calls agent.query() in a loop.

The script:
1. Asks Pivot to read code_bugged.py and create a fixed code_fixed.py
2. Runs the tests
3. If tests fail, sends the output back to Pivot
4. Repeats until all tests pass or max attempts reached

Usage:
    # With OpenRouter (needs OPENROUTER_API_KEY env var):
    python examples/example_2_auto_fix_loop/run_pivot.py

    # With a specific model:
    python examples/example_2_auto_fix_loop/run_pivot.py --model openrouter/google/gemini-2.5-flash

    # With scripted provider (no API, for demonstration):
    python examples/example_2_auto_fix_loop/run_pivot.py --scripted
"""
...
# Ensure pivotcode is importable from repo root
...
def clean_previous_solution() -> None:
    """Remove code_fixed.py from a previous run so Pivot starts fresh."""
...
def run_with_model(model: str) -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        print(f"Pivot: {answer[:400]}{'...' if len(answer) > 400 else ''}\n")
...
def run_scripted() -> None:
    ...
    clean_previous_solution()
    from pivotcode.providers.scripted_provider import ScriptedProvider, rule, text, tool_call
    ...
    agent = PivotCodeAgent(provider=provider, cwd=str(EXAMPLE_DIR), permission_mode="yolo")

    print("Attempt 1 (scripted)\n")
    answer = agent.query(
        "Fix the bugs in code_bugged.py → code_fixed.py. Don't modify the tests."
    )
    print(f"Pivot: {answer}\n")
